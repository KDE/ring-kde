...
//    /**/connect(m_pDefaultAccount,                 SIGNAL(clicked(bool))                  , this   , SLOT(changedAccountList())              );
   ...
   if (AccountModel::instance()->index(0,0).isValid()) {
...
void DlgAccounts::saveAccount(const QModelIndex& item)
{
   ...
   Account* account = AccountModel::instance()->getAccountByModelIndex(item);

   if(!account) {
      qDebug() << "Attempting to save details of an unexisting account : " << (item.data(Qt::DisplayRole).toString());
      return;
   }
...
//       AccountModel::instance()->setDefaultAccount(account);
//    }
...
void DlgAccounts::loadAccount(QModelIndex item)
{
   ...
   if ( ACC protocol() == Account::Protocol::SIP) {
      const QModelIndex idx = ACC credentialModel()->index(0,0);
      ...
      if (idx.isValid() && !account->id().isEmpty()) {
         edit5_password->setText( ACC credentialModel()->data(idx,CredentialModel::Role::PASSWORD).toString());
      }
      else
...
//    m_pDefaultAccount->setChecked(account == AccountModel::instance()->getDefaultAccount());
//    connect(m_pDefaultAccount,    SIGNAL(clicked(bool)) , this , SLOT(changedAccountList()) );
   ...
   list_credential->setModel( ACC credentialModel());
...
void DlgAccounts::accountListChanged(const QModelIndex& current, const QModelIndex& previous)
{
   ...
   if (previous.isValid()) {
      Account* acc = AccountModel::instance()->getAccountByModelIndex(previous);
...
void DlgAccounts::on_button_accountRemove_clicked()
{
   ...
   if (ret == KMessageBox::Yes) {
      AccountModel::instance()->remove(treeView_accountList->currentIndex());
      if (AccountModel::instance()->index(0,0).isValid())
         treeView_accountList->setCurrentIndex(AccountModel::instance()->index(0,0));
      loadAccount(treeView_accountList->currentIndex());
   }
...
void DlgAccounts::updateAccountListCommands()
{
   ...
   buttonsEnabled[1]     &= AccountModel::instance()->index(cur.row()+1,0).isValid();

   button_accountUp->setEnabled     ( true );
   button_accountDown->setEnabled   ( true );
   button_accountAdd->setEnabled    ( true );
   button_accountRemove->setEnabled ( true );
}
...
void DlgAccounts::updateFirstCredential(const QString& text)
{
   if (!m_IsLoading) {
      Account* acc = currentAccount();
      acc->credentialModel()->setData(acc->credentialModel()->index(0,0),text, CredentialModel::Role::NAME);
      if (acc->credentialModel()->index(0,0) == list_credential->currentIndex()) {
...
{
   currentAccount()->codecModel()->moveUp();
}

///Move codec down
void DlgAccounts::moveAudioCodecDown()
{
   currentAccount()->codecModel()->moveDown();
}
...
//       Account* current = AccountModel::instance()->getAccountAt(i);
//       current->updateState();
//    }
//    updateStatusLabel(treeView_accountList->currentIndex());
// }
...
void DlgAccounts::saveCredential()
{
   ...
   if (currentCredential.isValid()) {
      acc->credentialModel()->setData(currentCredential,edit_credential_auth->text()    , CredentialModel::Role::NAME     );
...
{
   list_credential->setCurrentIndex(currentAccount()->credentialModel()->addCredentials());
} //addCredential
...
void DlgAccounts::removeCredential() {
   Account* acc = currentAccount();
   acc->credentialModel()->removeCredentials(list_credential->currentIndex());
   list_credential->setCurrentIndex(acc->credentialModel()->index(0,0));
}
...
{
   return AccountModel::instance()->getAccountByModelIndex(treeView_accountList->currentIndex());
}
...
void DlgAccounts::updateSecurityValidation()
{
   ...
   //Create the widgets
   m_pVerifyServer->setModel(a->securityEvaluationModel());
   m_pVerifyClient->setModel(a->securityEvaluationModel());
   m_pReqTLS->setModel(a->securityEvaluationModel());
   m_pAuthorityII->setModel(a->securityEvaluationModel());
   m_pEndCertII->setModel(a->securityEvaluationModel());
   m_pPKII->setModel(a->securityEvaluationModel());
...
//    newWidget->addFlaw(new Flaw(SecurityEvaluationModel::SecurityFlaw::TLS_DISABLED,Certificate::Type::NONE));
   ...
   //Add the flaws
   /*foreach(const SecurityFlaw* flaw, currentAccount()->securityEvaluationModel()->currentFlaws()) {
      switch (flaw->flaw()) {
         case SecurityEvaluationModel::AccountSecurityChecks::SRTP_DISABLED:

            break;
         case SecurityEvaluationModel::AccountSecurityChecks::TLS_DISABLED:

            break;
         case SecurityEvaluationModel::AccountSecurityChecks::CERTIFICATE_EXPIRED:
            addFlawToCertificateField(flaw);
            break;
         case SecurityEvaluationModel::AccountSecurityChecks::CERTIFICATE_SELF_SIGNED:
            addFlawToCertificateField(flaw);
            break;
         case SecurityEvaluationModel::AccountSecurityChecks::CA_CERTIFICATE_MISSING:
            addFlawToCertificateField(flaw);
            break;
         case SecurityEvaluationModel::AccountSecurityChecks::END_CERTIFICATE_MISSING:
            addFlawToCertificateField(flaw);
            break;
         case SecurityEvaluationModel::AccountSecurityChecks::PRIVATE_KEY_MISSING:
            addFlawToCertificateField(flaw);
            break;
         case SecurityEvaluationModel::AccountSecurityChecks::CERTIFICATE_MISMATCH:

            break;
         case SecurityEvaluationModel::AccountSecurityChecks::CERTIFICATE_STORAGE_PERMISSION:

            break;
         case SecurityEvaluationModel::AccountSecurityChecks::CERTIFICATE_STORAGE_FOLDER:

            break;
         case SecurityEvaluationModel::AccountSecurityChecks::CERTIFICATE_STORAGE_LOCATION:

            break;
         case SecurityEvaluationModel::AccountSecurityChecks::OUTGOING_SERVER_MISMATCH:

            break;
         case SecurityEvaluationModel::AccountSecurityChecks::VERIFY_INCOMING_DISABLED:
            m_pVerifyServer->addFlaw(flaw);
            break;
         case SecurityEvaluationModel::AccountSecurityChecks::VERIFY_ANSWER_DISABLED:
            m_pVerifyClient->addFlaw(flaw);
            break;
         case SecurityEvaluationModel::AccountSecurityChecks::REQUIRE_CERTIFICATE_DISABLED:

            break;
         case SecurityEvaluationModel::AccountSecurityChecks::MISSING_CERTIFICATE:
            addFlawToCertificateField(flaw);
            break;
         case SecurityEvaluationModel::AccountSecurityChecks::MISSING_AUTHORITY:
            addFlawToCertificateField(flaw);
            break;
         case SecurityEvaluationModel::AccountSecurityChecks::COUNT__:
         default:
            qDebug() << "Invalid flaw";
      }
   }*/
