...
void DelegateDropOverlay::changeVisibility()
{
   foreach(const QModelIndex& idx, m_lActiveIndexes) {
      ...
      if (idx.isValid() && !idx.model()->rowCount(idx)) {
         int step = idx.data((int)Person::Role::DropState).toInt();
         //Remove items from the loop if there is no animation
         if (step >= 15 || step <= -15) {
            m_lActiveIndexes.removeAll(idx);
            if (step <= -15) //Hide the overlay
               ((QAbstractItemModel*)idx.model())->setData(idx,QVariant((int)0),(int)Person::Role::DropState);
         }
         else {
            //Update opacity
            step+=(step>0)?1:-1;
            ((QAbstractItemModel*)idx.model())->setData(idx,QVariant((int)step),(int)Person::Role::DropState);
         }
      }
   }
